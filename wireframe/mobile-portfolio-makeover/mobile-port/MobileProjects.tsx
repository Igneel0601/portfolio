...
/* Per-project presentation extras that don't live in the content model:
   the accent token each panel themes to (--ca), a one-line "insight" in
   Vaibhav's voice, and the status badge label. Keyed by project id so
   content.ts stays the single source of truth for everything else. */
type PanelExtra = { accent: string; badge: string; insight: ReactNode };
...
const FALLBACK: PanelExtra = { accent: "var(--accent)", badge: "shipped", insight: null };
        ...
        {PROJECTS.map((p) => {
          ...
          return (
            <article
              key={p.id}
              className="m-panel"
              style={{ ["--ca" as string]: x.accent }}
            >
              <div className="m-panel-meta">
              ...
              <p className="m-panel-tagline">{p.blurb}</p>
              {x.insight && <p className="m-panel-insight">{x.insight}</p>}

              <div className="m-panel-stack">
                {p.stack.map((s) => (
                  <span key={s} className="m-panel-chip">
                    {s}
                  </span>
                ))}
              </div>

              <Link href={`/work/${p.id}`} className="m-panel-cta">
                cat {p.id}.mdx
