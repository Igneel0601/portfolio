...
<AsciiDiagram caption="App Router · RSC pages · Server Actions · Prisma · Postgres" n="01">
{`
   ┌───────────────────────────────────────────────────────────┐
   │  Browser  (Next.js 16 · App Router · RSC)                 │
   │                                                           │
   │   Pages (RSC)        Client islands      Forms + charts   │
   │   src/app/(app)/*    Navbar, Dialogs     RHF + Zod        │
   │                                          Recharts         │
   └────────────┬──────────────────────────────────────────────┘
   ...
   │   src/server/actions/*     ← mutations (trips, stops,     │
   │                              expenses, notes, packing)    │
   │   src/lib/auth.ts          ← Auth.js v5 (credentials)     │
   │   src/lib/server-action-utils ← safeAction() envelope     │
   └────────────┬──────────────────────────────────────────────┘
                │ Prisma ORM
                ▼
   ┌───────────────────────────────────────────────────────────┐
   │  PostgreSQL · 12 tables · User > Trip > Stop > Activity   │
   └───────────────────────────────────────────────────────────┘
`}
</AsciiDiagram>
...
<Steps>
  <Step n="01" label="landing → login" hint="~3s">
    <Figure
      src="/projects/traveloop.png"
      alt="Traveloop landing page"
      placeholder="full-bleed hero with cta into the app"
    />
  </Step>
  <Step n="02" label="dashboard" hint="welcome stats">
    <Figure placeholder="welcome stats and recent trips" />
  </Step>
  <Step n="03" label="trip builder" hint="add stops + activities">
    <Figure placeholder="itinerary view, day-grouped activities" />
  </Step>
  <Step n="04" label="budget pie" hint="recharts">
    <Figure placeholder="per-category pie + bar with over-budget warning" />
  </Step>
</Steps>
